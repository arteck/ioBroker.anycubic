...
class WebsocketController {
    ...
    initWsClient() {
        try {
            ...
            // No 'open' listener here — consolidated into start() to avoid duplicate listeners.
            // info.connection=true is only set on a successful pong response (see 'pong' handler).
            ...
            this.wsClient.on('pong', () => {
                ...
                this.adapter.setStateChanged('info.connection', true, true);
            ...
            this.wsClient.on('close', () => {
                ...
                if (!this._noReconnect) {
                    this.adapter.setStateChanged('info.dataError', 'WebSocket connection closed unexpectedly', true);
                }
            ...
            this.wsClient.on('error', (err) => {
                ...
                this.adapter.log.debug(`<anycubic> WebSocket error: ${err.message}`);
                this.adapter.setStateChanged('info.connection', false, true);
                this.adapter.setStateChanged('info.dataError', err.message, true);
            });
    ...
    _checkPongTimeout() {
        ...
        if (this._missedPongs >= this._maxMissedPongs) {
            ...
            this.adapter.setStateChanged('info.dataError', `Printer unreachable: ${this._missedPongs} missed pongs`, true);
            if (this.wsClient) {
    ...
    async autoRestart() {
        ...
        this.autoRestartTimeout = setTimeout(async () => {
            // Re-check energy_id before starting — energy may have been turned off during backoff
            if (this.adapter.config.energy_id) {
                try {
                    const energyState = await this.adapter.getStateAsync(this.adapter.config.energy_id);
                    if (energyState && energyState.val === false) {
                        this.adapter.log.debug('<anycubic> Energy is off - not reconnecting websocket (backoff expired).');
                        this._retryCount = 0;
                        return;
                    }
                } catch (e) {
