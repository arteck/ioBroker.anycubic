...
const { EXCEPTIONS, BLACKLIST } = require('./constants');
...
class Helper {
    ...
    /**
     * Prüft ob ein Pfad auf der Blacklist steht.
     * Ein Pfad wird geblockt wenn:
     * - er exakt einem BLACKLIST-Eintrag entspricht,
     * - er mit einem BLACKLIST-Eintrag als Präfix beginnt (z.B. "polygon.x"),
     * - ODER ein beliebiges dot-separiertes Segment exakt einem BLACKLIST-Eintrag
     *   entspricht (z.B. "exclude_object.objects.0.polygon").
     *
     * @param {string} path - Der vollständige State-Pfad (z.B. "exclude_object.objects.0.polygon")
     * @returns {boolean} - true wenn der Pfad NICHT angelegt werden soll
     */
    _isBlacklisted(path) {
        ...
        for (const entry of BLACKLIST) {
            // Match wenn Pfad genau dem Eintrag entspricht ODER mit "entry." beginnt
            if (path === entry || path.startsWith(`${entry}.`)) {
                return true;
            }
            // Match wenn ein beliebiges Segment exakt dem Eintrag entspricht
    ...
    async parse(path, element, options = {write: false}, change = false) {
        ...
        // Blacklist-Prüfung: Pfade die auf der Blacklist stehen komplett ignorieren
        if (this._isBlacklisted(parsePath)) {
            this.adapter.log.debug(`Blacklisted path skipped: ${parsePath}`);
            return;
        }
        ...
        if (typeof element === "string" || typeof element === "number" || typeof element === "boolean") {
            ...
            if (!this.alreadyCreatedObjects[parsePath]) {
                try {
                    ...
                    this.adapter.setStateChanged('info.dataError', `Parse error: ${error.message || error}`, true);
                }
            }
            ...
            if (this.isObject(valDP) && !Array.isArray(valDP) && Object.keys(valDP).length === 0) {
                if (!this.alreadyCreatedObjects[fullPath]) {
                    try {
                        ...
                        this.adapter.setStateChanged('info.dataError', `Parse error: ${error.message}`, true);
                    }
                }
                continue;
            }
    ...
    async extractArray(element, key, path, options) {
        try {
            ...
            this.adapter.setStateChanged('info.dataError', `Array extraction error: ${error.message || error}`, true);
        }
    }
